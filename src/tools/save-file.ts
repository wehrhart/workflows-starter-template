/**
 * Save a Blob as a named file in ONE user gesture — including from inside the
 * hosted-artifact iframe, whose sandbox blocks page-initiated downloads.
 *
 * Top-level pages get the classic invisible-<a download> click. Embedded pages
 * route through a "trampoline": a tiny HTML page opened via window.open (popups
 * may escape the sandbox even where downloads cannot) that auto-clicks its own
 * <a download> — outside the sandbox the download proceeds, with the right
 * filename — then closes itself. If the popup is blocked we still attempt the
 * direct click.
 *
 * Object URLs are revoked on a delay, not immediately: the trampoline tab needs
 * the blob to stay resolvable until its own click fires.
 */
